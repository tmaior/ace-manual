...
- **Structure**: Documentation is organized by **directories and subdirectories**. Each doc belongs in the **correct** folder (e.g. central docs: `rules/`, `architecture/`, `environments/`, `services/` under the docs root; or service-level: `ace-db-gateway/docs/api/`). Follow the existing layout; do not invent new top-level folders without aligning with the project.
...
**Agents**: Use kebab-case for new filenames. Place new docs in the right directory (e.g. `api/`, `development/`, `architecture/` under a service’s `docs/` or under the central docs root: `rules/`, `architecture/`, etc.). After any structural change, update `index.md`, `README.md`, and `START_HERE.md` as applicable.
